...
struct FlyingOrb: Identifiable, Equatable, Sendable {
    let id = UUID()
    let fromRow: Int, fromColumn: Int
    let toRow: Int, toColumn: Int
    let player: Int
    var arrived = false
}
...
final class NeuralAISolver: @unchecked Sendable {
    ...
    private func loadModelDetail(named name: String, config: MLModelConfiguration) -> (MLModel, String)? {
        if let url = Bundle.main.url(forResource: name, withExtension: "mlmodelc"),
           let m = try? MLModel(contentsOf: url, configuration: config) { return (m, "OK") }
        let all = Bundle.main.urls(forResourcesWithExtension: "mlmodelc", subdirectory: nil) ?? []
        if let u = all.first(where: { $0.deletingPathExtension().lastPathComponent == name }),
           let m = try? MLModel(contentsOf: u, configuration: config) { return (m, "OK") }
        if let raw = Bundle.main.url(forResource: name, withExtension: "mlmodel"),
           let c = try? MLModel.compileModel(at: raw),
           let m = try? MLModel(contentsOf: c, configuration: config) { return (m, "OK") }
        return nil
    }

    func getBestMove(flatBoard: [AISolver.SimCell], boardSize: Int, currentPlayer: Int,
                     playerType: PlayerType, validMoves: [(Int, Int)],
        ...
        guard boardSize <= 12 else {
            Task { @MainActor in ModelStatus.shared.activeWarning = "⚠️ 棋盘尺寸超出神经网络限制，降级为启发式" }
            return fallback() ?? validMoves.randomElement()
        }
        guard let model = activeModel else {
            Task { @MainActor in ModelStatus.shared.activeWarning = "⚠️ \(playerType.rawValue) 未加载，降级为启发式" }
            return fallback() ?? validMoves.randomElement()
        }
        ...
        do {
            ...
            let shape: [NSNumber] = [1, NSNumber(value: numChannels), 12, 12]
            ...
            for r in 0..<boardSize {
                for c in 0..<boardSize {
                    ...
                    if let owner = cell.owner {
                        ...
                        ptr[rel * channelStride + r * maxDim + c] = Float(cell.count) / 4.0
                    }
                }
            }
            if isModel1 {
            ...
            let inputKey: String
            if inDescs[explicitIn] != nil { inputKey = explicitIn }
            else if inDescs.count == 1, let only = inDescs.keys.first { inputKey = only }
            else {
                Task { @MainActor in ModelStatus.shared.activeWarning = "❌ 模型缺少输入节点 \(explicitIn)" }
                return fallback() ?? validMoves.randomElement()
            }
            ...
            if outDescs[explicitOut] != nil { outputKey = explicitOut }
            else if outDescs.count == 1, let only = outDescs.keys.first { outputKey = only }
            else {
                Task { @MainActor in ModelStatus.shared.activeWarning = "❌ 模型缺少输出节点 \(explicitOut)" }
                return fallback() ?? validMoves.randomElement()
            }

            guard let logits = prediction.featureValue(for: outputKey)?.multiArrayValue else {
                Task { @MainActor in ModelStatus.shared.activeWarning = "❌ 无法读取模型输出 \(outputKey)" }
                return fallback() ?? validMoves.randomElement()
            }
            ...
            for (r, c) in validMoves {
                ...
                if s > maxScore { maxScore = s; bestMove = (r, c) }
            }
            return bestMove
        } catch {
            let msg = error.localizedDescription
            Task { @MainActor in ModelStatus.shared.activeWarning = "❌ CoreML 推理失败: \(msg)" }
            return fallback() ?? validMoves.randomElement()
        }
    }
}

// MARK: - AI 解算器（算法部分与上一版一致）
struct AISolver: Sendable {
    struct SimCell: Sendable { var owner: Int?; var count: Int }
    struct MoveKey: Hashable, Sendable { let r: Int; let c: Int }
    ...
    struct Board: Sendable {
        ...
        init(flat: [SimCell], size: Int) { self.size = size; self.cells = flat }
    ...
    final class MCTSNode {
        ...
        init(move: (Int, Int)?, player: Int, board: Board, unvisitedMoves: [(Int, Int)],
             hasEntered: [Bool], eliminated: [Bool], parent: MCTSNode? = nil) {
            self.move = move; self.player = player; self.boardState = board
            self.unvisitedMoves = unvisitedMoves; self.hasEnteredState = hasEntered
            self.eliminatedState = eliminated; self.parent = parent
        }
    ...
    init(boardSize: Int, activePlayers: [Int], currentPlayer: Int, playerType: PlayerType,
         board: [[Cell]], hasEntered: [Bool], eliminated: [Bool]) {
        ...
        for row in board { for cell in row { flat.append(SimCell(owner: cell.owner, count: cell.count)) } }
    ...
    func calculateBestMove() async -> (Int, Int)? {
        ...
        if playerType.isNeuralAI {
            ...
            return NeuralAISolver.shared.getBestMove(
                flatBoard: flat, boardSize: boardSize, currentPlayer: currentPlayer,
                playerType: playerType, validMoves: validMoves,
        ...
        if board.totalOrbs < 6 {
            ...
            if !pruned.isEmpty { validMoves = pruned }
        }

        let isMultiplayer = activePlayers.count > 2
        switch playerType {
        case .mathEasy: return validMoves.randomElement()
        case .mathNormal: return heuristicBestMove(validMoves: validMoves)
        case .mathHard:
            return isMultiplayer ? await runMCTS(iterations: 600, isBiased: false, validMoves: validMoves)
                                 : getMaxBestimMinimaxMove(depthizing: 2, validMoves: valid:Moves)
        case . falsemathExpert:
            return, isMultiplayer ? turn await runMCTS(Playeriterations: :1200, isBi currentased: true, validMoves: validMoves)
                                 : getBestMinimaxMove(depth: 3, validMoves: validMoves)
        case .mathUltimate:
            return await runMCTS(iterations: 1500, isBiased: true, validMoves: validMoves)
        case .mathNightmare:
            return isMultiplayer ? await runMCTS(iterations: 3000, isBiased: true, validMoves: validMoves)
                                 : calculateNightmareMove(validMoves: validMoves)
        default: return validMoves.randomElement()
        }
    }
    ...
    private func heuristicBestMove(validMoves: [(Int, Int)]) -> (Int, Int)? {
        ...
        guard let maxScore = scored.map({ $0.1 }).max() else { return validMoves.randomElement() }
    ...
    private func calculateNightmareMove(validMoves: [(Int, Int)]) -> (Int, Int)? {
        ...
        for move in sorted {
            ...
            let s = minimaxNightmare(boardState: board, hasEnteredState: hasEntered, eliminatedState: eliminated,
                                     move: move, depth: depth, alpha: Int.min + 1, beta: Int.max - 1,
                                     isPlayer)
            if s > bestScore { bestScore = s; bestMoves = [move] }
            else if s == bestScore { bestMoves.append(move) }
        }
        return bestMoves.randomElement() ?? sorted.first
    }

    private func minimaxNightmare(boardState: Board, hasEnteredState: [Bool], eliminatedState: [Bool],
                                  move: (Int, Int), depth: Int, alpha: Int, beta: Int,
                                  isMaximizing: Bool, turnPlayer: Int) -> Int {
        if Task.isCancelled { return 0 }
        var b = boardState, h = hasEnteredState, e = eliminatedState
        ...
        let np = getNextPlayerSimulated(current: turnPlayer, eliminated: e)
        let nextMoves = getPlayableCells(for: np, on: b)
        if nextMoves.isEmpty { return evaluateNightmareBoard(b, forPlayer: currentPlayer, eliminated: e) }
        let sorted = nextMoves.sorted {
            evaluateTacticalMoveScore($0, on: b, player: np) > evaluateTacticalMoveScore($1, on: b, player: np)
        }
        var a = alpha, betaV = beta
        if isMaximizing {
            var maxEval = Int.min
            for m in sorted {
                let v = minimaxNightmare(boardState: b, hasEnteredState: h, eliminatedState: e,
                                         move: m, depth: depth - 1, alpha: a, beta: betaV,
                                         isMaximizing: false, turnPlayer: np)
                maxEval = max(maxEval, v); a = max(a, v)
                if betaV <= a { break }
            }
            return maxEval
        } else {
            ...
            for m in sorted {
                let v = minimaxNightmare(boardState: b, hasEnteredState: h, eliminatedState: e,
                                         move: m, depth: depth - 1, alpha: a, beta: betaV,
                                         isMaximizing: true, turnPlayer: np)
                minEval = min(minEval, v); betaV = min(betaV, v)
                if betaV <= a { break }
            }
            return minEval
        }
    ...
    private func getBestMinimaxMove(depth: Int, validMoves: [(Int, Int)]) -> (Int, Int)? {
        ...
        for move in validMoves {
            if Task.isCancelled { break }
            let s = minimaxRecursive(boardState: board, hasEnteredState: hasEntered, eliminatedState: eliminated,
                                     move: move, depth: depth, alpha: Int.min, beta: Int.max,
                                     isMaximizing: false, turnPlayer: currentPlayer)
            if s > bestScore { bestScore = s; bestMoves = [move] }
            else if s == bestScore { bestMoves.append(move) }
        }
    ...
    private func minimaxRecursive(boardState: Board, hasEnteredState: [Bool], eliminatedState: [Bool],
                                  move: (Int, Int), depth: Int, alpha: Int, beta: Int,
                                  isMaximizing: Bool, turnPlayer: Int) -> Int {
        if Task.isCancelled { return 0 }
        var b = boardState, h = hasEnteredState, e = eliminatedState
        ...
        let np = getNextPlayerSimulated(current: turnPlayer, eliminated: e)
        let nextMoves = getPlayableCells(for: np, on: b)
        if nextMoves.isEmpty { return evaluateEnhancedBoard(b, forPlayer: currentPlayer, eliminated: e) }
        var a = alpha, betaV = beta
        if isMaximizing {
            var maxEval = Int.min
            for m in nextMoves {
                let v = minimaxRecursive(boardState: b, hasEnteredState: h, eliminatedState: e,
                                         move: m, depth: depth - 1, alpha: a, beta: betaV,
                                         isMaximizing: false, turnPlayer: np)
                maxEval = max(maxEval, v); a = max(a, v)
                if betaV <= a { break }
    ...
    private func runMCTS(iterations: Int, isBiased: Bool, validMoves: [(Int, Int)]) async -> (Int, Int)? {
        ...
        let merged = await withTaskGroup(of: [MoveKey: Int].self) { group in
            for _ in 0..<taskCount {
                group.addTask {
                    let root = MCTSNode(move: nil, player: self.currentPlayer, board: self.board,
                                        unvisitedMoves: validMoves, hasEntered: self.hasEntered,
                                        eliminated: self.eliminated)
                    for _ in 0..<perTask {
                        if Task.isCancelled { break }
                        var node = root
                        while node.isFullyExpanded && !node.children.isEmpty {
                            if let nx = node.selectBestChildUCB() { node = nx } else { break }
                        }
                        if !node.unvisitedMoves.isEmpty {
                            let move = node.unvisitedMoves.remove(at: Int.random(in: 0..<node.unvisitedMoves.count))
                            var b = node.boardState, h = node.hasEnteredState, e = node.eliminatedState
                            ...
                            let child = MCTSNode(move: move, player: np, board: b,
                                                 unvisitedMoves: self.getPlayableCells(for: np, on: b),
                                                 hasEntered: h, eliminated: e, parent: node)
            ...
            for await v in group { for (k, n) in v { total[k, default: 0] += n } }
            return total
        }
        if let best = merged.max(by: { $0.value < $1.value })?.key { return (best.r, best.c) }
        return validMoves.randomElement()
    }

    private func simulatePlayout(from node: MCTSNode, isBiased: Bool) -> Int? {
        var b = node.boardState, h = node.hasEnteredState, e = node.eliminatedState
        var cur = node.player, step = 0
        let maxSteps = boardSize >= 9 ? 25 : 50
        while step < maxSteps {
            step += 1
            let moves = getPlayableCells(for: cur, on: b)
            if moves.isEmpty { e[cur] = true }
            else {
                let move: (Int, Int)
        ...
        for r in 0..<boardSize {
            for c in 0..<boardSize {
                if let o = b[r, c].owner { scores[o] += b[r, c].count + (b[r, c].count == 3 ? 5 : 0) }
            }
        }
    ...
    private func biasedPick(_ moves: [(Int, Int)], on b: Board) -> (Int, Int)? {
        ...
        guard let maxScore = scored.map({ $0.1 }).max() else { return moves.randomElement() }
        return scored.filter { $0.1 == maxScore }.randomElement()?.0
    }

    private func simulateMoveBFS(on simBoard: inout Board, move: (Int, Int), player: Int, hasEntered: inout [Bool]) {
        let (r, c) = move
    ...
    private func evaluateNightmareBoard(_ b: Board, forPlayer player: Int, eliminated: [Bool]) -> Int {
        ...
        for r in 0..<boardSize {
            for c in 0..<boardSize {
                ...
                if isCorner { v += EvalWeights.cornerBonus } else if isEdge { v += EvalWeights.edgeBonus }
                if cell.count == 3 {
                    v += EvalWeights.threeStackBonus
                    for (nr, nc) in neighbors(of: r, c) {
                        if let ao = b[nr, nc].owner, ao != owner, b[nr, nc].count == 3 {
                            v += (owner == player ? EvalWeights.adjacentThreeAllyBonus
                                                  : EvalWeights.adjacentThreeEnemyPenalty)
                        }
                    }
                }
                if owner == player { myOrbs += cell.count; score += v }
                else { enemyOrbs += cell.count; score -= Int(Double(v) * EvalWeights.enemyMultiplier) }
            }
        }
        let eliminatedEnemies = activePlayers.filter { $0 != player && eliminated[$0] }.count
        return score + eliminatedEnemies * EvalWeights.eliminatedEnemyBonus
    ...
    private func evaluateEnhancedBoard(_ b: Board, forPlayer player: Int, eliminated: [Bool]) -> Int {
        ...
        for r in 0..<boardSize {
            for c in 0..<boardSize {
                ...
                let isCorner = (r == 0 || r == boardSize - 1) && (c == 0 || c == boardSize - 1)
                let isEdge = r == 0 || r == boardSize - 1 || c == 0 || c == boardSize - 1
                let pos = isCorner ? EvalWeights.enhancedCornerBonus
                                   : (isEdge ? EvalWeights.enhancedEdgeBonus : 0)
                let v = cell.count * EvalWeights.enhancedOrbValue + pos
                      + (cell.count == 3 ? EvalWeights.enhancedThreeStack : 0)
                if owner == player { myCount += cell.count; score += v }
                else { enemyCount += cell.count; score -= Int(Double(v) * EvalWeights.enhancedEnemyMultiplier) }
            }
        }
        let eliminatedEnemies = activePlayers.filter { $0 != player && eliminated[$0] }.count
        return score + eliminatedEnemies * EvalWeights.enhancedEliminatedEnemy
    ...
    private func updateEliminatedStatus(board b: Board, hasEntered: [Bool], eliminated: inout [Bool]) {
        ...
        for i in 0..<cells.count {
            if let o = cells[i].owner, o >= 0, o < 4 { appears[o] = true }
        }
        for p in 0..<4 where hasEntered[p] && !eliminated[p] {
            if !appears[p] { eliminated[p] = true }
        }
    }
    ...
    private func getPlayableCells(for player: Int, on b: Board) -> [(Int, Int)] {
        var ownsAny = false
        let cells = b.cells
        for i in 0..<cells.count { if cells[i].owner == player { ownsAny = true; break } }
        var moves: [(Int, Int)] = []
        moves.reserveCapacity(b.size * b.size)
        for r in 0..<boardSize {
            for c in 0..<boardSize {
                let owner = b[r, c].owner
                if ownsAny ? owner == player : owner == nil { moves.append((r, c)) }
            }
        }
        return moves
    }
...
final class GameModel: ObservableObject {
    ...
    private func resetTransientState() {
        emphasizedCells = []; flyingOrbs = []; playableCells = []
    ...
    func returnToMenu() {
        cancelAllTasks()
        resetTransientState()
        isChoosingStarter = false; isResolving = false
        showMenu = true
    ...
    private func chooseStarter() async {
        ...
        do {
            ...
            for tick in 0..<totalTicks {
                ...
                let p = Double(tick) / Double(totalTicks)
                try await Task.sleep(nanoseconds: UInt64((45 + Int(p * p * 140)) * 1_000_000))
            }
    ...
    func play(row: Int, column: Int) {
        let idx = row * boardSize + column
        guard !showMenu, !isChoosingStarter, winner == nil, !isResolving,
              !eliminated[currentPlayer], playableCells.contains(idx),
        ...
        do {
            ...
            while !wave.isEmpty {
                ...
                for (r, c) in exploding {
                    for (nr, nc) in neighbors(of: r, c) {
                        transfers.append(FlyingOrb(fromRow: r, fromColumn: c, toRow: nr, toColumn: nc, player: player))
                    }
                }
                ...
                withAnimation(.easeOut(duration: 0.16)) {
                    for i in flyingOrbs.indices { flyingOrbs[i].arrived = true }
                }
                ...
                wave = changed.compactMap {
                    let r = $0 / boardSize, c = $0 % boardSize
            ...
            emphasizedCells = []
            isResolving = false
            if winner == nil { advanceTurn(); rebuildPlayableCells(); triggerAIMoveIfNeeded() }
        } catch {
        ...
        aiTask = Task { [weak self] in
            ...
            do {
                ...
                let bestMove = await Task.detached(priority: .userInitiated) {
                    let solver = AISolver(boardSize: bSize, activePlayers: active, currentPlayer: currP,
                                          playerType: pType, board: currentBoard,
                                          hasEntered: enteredState, eliminated: elimState)
    ...
    var body: some View {
        VStack(spacing: 20) {
            Image(systemName: "circle.grid.3x3.fill")
                .font(.system(size: 56)).symbolRenderingMode(.hierarchical)
            Text("Color War")
            ...
            VStack(spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ModelStatusRow(name: "ColorWarAI (5通道)",
                                   isReady: status.isModel1Ready, message: status.model1Message)
                    ModelStatusRow(name: "colorwar_12x12 (4通道)",
                                   isReady: status.isModel2Ready, message: status.model2Message)
                }
                .padding(12)
                ...
                VStack(spacing: 6) {
                    ...
                    Slider(value: Binding(get: { Double(boardSize) },
                                          set: { boardSize = Int($0) }), in: 5...12, step: 1)
                        .tint(.indigo)
                }
        ...
        GeometryReader { proxy in
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(game.flyingOrbs) { orb in
                    // 飞行珠：也改成玻璃质感（无交互）
                    GlassPiece(color: game.colors[orb.player], count: 1,
                               size: max(8, side * 0.40), interactive: false)
                        .position(
...
struct CellView: View, Equatable {
    ...
    static func == (lhs: CellView, rhs: CellView) -> Bool {
        lhs.cell == rhs.cell && lhs.color == rhs.color
            && lhs.active == rhs.active && lhs.isPlayable == rhs.isPlayable
    }
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onTapGesture { if isPlayable { onTap() } }
...
// MARK: - 玻璃棋子（关键改动）
// 使用 iOS 26 液态玻璃按钮视觉 + 触摸下沉（.interactive()）
...
struct WinnerView: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            HStack {
                Button("返回菜单") { game.returnToMenu() }
                    .buttonStyle(.glass)
                Button("再来一局") { game.prepareGame(size: game.boardSize) }
                    .buttonStyle(.glassProminent)
            }
        }
        .padding(32)
